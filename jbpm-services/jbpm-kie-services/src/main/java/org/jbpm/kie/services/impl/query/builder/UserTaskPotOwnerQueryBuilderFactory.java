...
public class UserTaskPotOwnerQueryBuilderFactory implements QueryParamBuilderFactory{
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(UserTaskPotOwnerQueryBuilderFactory.class);
    
    @Override
    public boolean accept(String identifier) {
        //LOGGER.debug("Entro en ACCEPT: {}",identifier);
        
        if ("potOwnerBuilder".equalsIgnoreCase(identifier)) {
            //LOGGER.debug("Encontrado: {}",identifier);
            return true;
        }
